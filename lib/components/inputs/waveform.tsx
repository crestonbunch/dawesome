...
import "components/inputs/waveform.scss";
...
export default class Waveform extends React.PureComponent<WaveformProps, undefined> {
    ...
    calcPoint(x: number) {
        ...
        this.props.partials.forEach((amplitude, partial) => {
            y += amplitude * Math.cos(2 * Math.PI * x * this.props.freq * partial);
        });
        const r = {x: x, y: y};
        return r;
    }
